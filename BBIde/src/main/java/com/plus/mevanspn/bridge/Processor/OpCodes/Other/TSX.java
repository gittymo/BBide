...
/** The TSX class is used to create objects representing the TXS assembler mnemonic in
 * BBide programs.  The purpose of this mnemonic is to transfer the contents of the
 * Stack Pointer to the X register.
 * (Z) flag is set if X register value becomes zero.
 * (N) flag is set if bit seven of the X register is set.
...
public class TSX extends OpCode {
	...
	public void perform(Memory memory) throws InvalidAddressModeException, InvalidAddressException, MemoryMissingException {
		...
		memory.registers.replace("X", memory.registers.get("SP"));
